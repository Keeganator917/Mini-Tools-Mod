...
public class EnderSlayerSword extends Item {
    public EnderSlayerSword(ToolMaterial toolMaterial, float attackDamage, float attackSpeed, Settings settings) {
    ...
    public ActionResult use(World world, PlayerEntity user, Hand hand) {
        ...
        if (world instanceof ServerWorld serverWorld) {
            ProjectileEntity.spawnWithVelocity(EnderPearlEntity::new, serverWorld, itemStack, user, 0.0F, 1.5f, 1.0F)
                    .setItem(Items.ENDER_PEARL.getDefaultStack());
        }

        user.incrementStat(Stats.USED.getOrCreateStat(this));
        itemStack.damage(10, user);
